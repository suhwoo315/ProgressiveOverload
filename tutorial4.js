class Tutorial4{
    constructor(){
        ...
        this.dialogue = ["레인보우 바벨을 뽑았던 첫 번째 동작 '덤벨 컬'은\n바벨로 머슬로스를 공격하게 해줍니다.", //0
                        ...
                        "프로틴 우물을 재가동시켰던 두 번째 동작 '레터럴 레이즈'는\n프로틴 쉐이크로 머슬로스를 공격하게 해줍니다.", //2
                        "화면에 해당 아이콘이 뜨면 '레터럴 레이즈'로 머슬로스를 공격하세요!", //3
                        ...
                        "양손에 덤벨을 하나씩 들어주세요.", //9
                        "준비 되었나요? 건투를 빕니다!"]; //10
        this.exercises = ["덤벨 컬", "레터럴 레이즈", "숄더 프레스"];
    ...
    display(){
        ...
        switch(this.cut){
            ...
            case 4:
                ...
                // 캐릭터 모션
                image(tutorial4_chr[this.cut], width/2, height/2, width, height);
                // image(tutorial4_chr[this.cut + 1], width/2, height/2, width, height);
                //스페이스바
                if (frameCount % 60 < 30) image(map1_chr[1], width * 63 / 100, height * 63 / 100, width * 0.6, height * 0.6);
                else image(map1_chr[2], width * 63 / 100, height * 63 / 100, width * 0.6, height * 0.6);
                break;
